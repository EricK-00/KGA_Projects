...
* private 멤버는 상속도 할 수 없다. protected 멤버는 자식 클래스까지만 접근할 수 있다
* 
* 메서드 오버라이드(메서드 재정의) - 부모 클래스의 메서드와 동일한 이름으로 자식 클래스에서 재정의
* 부모 클래스의 메서드에는 virtual 키워드(가상 메서드), 자식 클래스의 메서드에는 override 키워드
* 오버로딩 - 같은 이름의 메서드를 다른 매개변수로 생성 / 오버라이딩 - 자식 클래스에서 부모 클래스의 메서드 재정의
* 
* 프로퍼티(속성) - 괄호가 없는 메서드. get/set으로 읽기/쓰기. 액세스 한정자로 읽기, 쓰기 접근을 각각 제한할 수 있다
* 
* 인터페이스 - 명세서 역할을 한다. 인터페이스를 상속받은 클래스는 인터페이스에 선언된 멤버가 반드시 구현되어야 한다
* 인터페이스는 직접 인스턴스화 되지 않고, 다중 상속을 지원한다
* 인터페이스의 멤버는 항상 public이고, virtual, static을 붙일 수 없다
* 추상 클래스와 인터페이스는 다른 클래스에 멤버 이름을 미리 정의할 때 사용한다. 둘의 차이는 인터페이스는 멤버 이름만 정의하고 내용은 구현하지 않는다
* C++에서는 순수가상함수
* 
* 객체지향 프로그래밍 - 캡슐화, 상속, 다형성, 추상화. 프로그램 분석, 유지보수, 재사용성이 쉬워진다
* 개체 - 클래스로 새로운 형식을 정의하고, 개체는 데이터와 기능을 숨기는 캡슐화 기능 제공
* 
* 추상화 - 추상 클래스 또는 인터페이스에서 상속받아 구현하는 설계 관련 개념. 공통 특성을 집합해서 인터페이스로 작성. 의존성 감소
* 캡슐화 - 필드와 메서드를 하나의 클래스로 묶어 놓고, 액세스 제한으로 내부 필드나 로직은 숨기는 개념. 외부에는 필요한 기능만 제공한다
* 상속 - 멤버를 자식 클래스에서 재사용하는 개념
* 다형성 - 같은 이름의 메서드가 상황에 따라 다르게 동작하는 개념
* 
* 배열 - 데이터를 묶어서 관리할 수 있다. 크기가 고정되어 있다
* 컬렉션 클래스 - 리스트(list), 딕셔너리(dictionary), 셋(set), 맵(map), 스택(stack), 큐(queue), 링크드 리스트(linked list)
* 딕셔너리 - 레드블랙트리로 구현
* 
* 대리자 - 시그니처가 동일한 메서드들을 참조를 통해 보유한다. 보유한 메서드를 대신 실행할 수 있다(Invoke())
* Action - 반환값 없음, Func - 반환값 있음(맨 왼쪽 타입이 반환타입)
* 
* 메서드의 매개변수 전달 방식
* - 값 전달 방식: 값 그대로 복사해서 전달
* - ref 키워드(참조 전달 방식): 호출된 메서드에 참조로 전달
* - out 키워드(반환형 전달 방식): 호출하는 쪽에서 초기화하지 않고 전달하고, 호출된 메서드에서 데이터를 초기화해서 반환
* - params 키워드(가변형 전달 방식): 매개변수를 가변적으로 전달
* 
*/
